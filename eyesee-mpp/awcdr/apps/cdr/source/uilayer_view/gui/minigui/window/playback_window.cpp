...
void PlaybackWindow::keyProc(int keyCode, int isLongPress) 
{
	...
	switch (keyCode) {
		...
		case SDV_KEY_MODE: {
			//Album previous page ; fast rewind ;previous video 
			...
			if (!IsPlayingWindow()) {
				...
				if (isLongPress == LONG_PRESS) { //handle fling update page
					...
					//handle button update pre select
					if(select_file_ > 0)
					 {
						select_file_--;
		...
		case SDV_KEY_RIGHT: {//unlock and locked ; Next video ;fast forward 
		...
		case SDV_KEY_OK: { //Album next page ;delete video
		...
		, ignore_message_flag_(false)
{
	...
	ivplayback_thumb_->RemoveAllIconItem();
	int item_width = 240; // SCREEN_HEIGHT / 4;	// 213
	int item_height = 120 + 44; // (SCREEN_WIDTH - 120 * 2) / 2;
...
void PlaybackWindow::UpdateFileList(const vector<string> file_list, int sum, bool reset_flag)
{
	...
	db_warn("file_sum_: %d, page_sum_: %d, page_num_: %d", file_sum_, page_sum_, page_num_);
	if (reset_flag)
		ResetFlagStatus();
	#if 0
	vector<string>::const_iterator c_it;
...
Play:
		if (player_status_ == STOPED || player_status_ == COMPLETION) //if completion ,replay .if stoped ,start play
			if (select_file_ % IMAGEVIEW_NUM) {
				UpdateListView(false);
			} else {
				UpdateListView(true);
...
int PlaybackWindow::FileLockCtl()
{
		...
		||mediaf_image_[index].fileType == VIDEO_A || mediaf_image_[index].fileType == VIDEO_B)
	{
		...
		db_debug("==========index %d playlist_ file name %s==========", index, playlist_[select_file_].c_str());
		...
		db_warn("current file index %d type %d is not the video type !!!", index, mediaf_image_[index].fileType);
		ret = -1;
	}
out:
...
int PlaybackWindow::FileLockCtl(int index)
{
		...
		||(itr->second).fileType == VIDEO_A || (itr->second).fileType == VIDEO_B )
	{
		...
		if ((itr->second).fileType == VIDEO_A) { // 20190612_092811.ts -> 20190612_092811_SOS.ts
			...
			//ivplayback_thumb_->SetIconItem_Icon(&icon_filetype_[6], index, 2);
			ivplayback_thumb_->SetIconItem_lock(index, 1);
		} else if((itr->second).fileType == VIDEO_A_SOS) { 
			// video/F/20190612_092811_SOS.ts -> 20190612_092811.ts
		...
		db_debug("==========index %d playlist_ file name %s==========", index, playlist_[index].c_str());
		m_update_file_name.clear();
		m_update_file_name = file_name_bak.c_str();
		//mv the ths.jpg and ts
		snprintf(buf, sizeof(buf) - 1, "mv %s %s", file_name.c_str(), file_name_bak.c_str());
		...
		//change the mediaf_image fileType
		if((itr->second).fileType == VIDEO_A_P) {
			// event/F/20190612_092811_PARK.ts	-> 20190612_092811.ts
			(itr->second).fileType = VIDEO_A;		
			file_type = "video_A";
			lock_status = 0;
			#ifdef USE_CAMB
		...
		db_warn("current file index %d type %d is not the video type !!!", index, (itr->second).fileType);
		ret = -1;
	}
out:
	pthread_mutex_unlock(&m_lock_file_ctl);
...
{
	#ifdef UPDATEIMGUSETHREAD
	db_info("UpdateImgRange %d lock: %d lastupdate: %d listview_flag_: %d", start, lockxx, lastupdate, listview_flag_);
