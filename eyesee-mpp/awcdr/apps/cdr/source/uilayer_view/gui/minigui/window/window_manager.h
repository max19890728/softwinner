...
class WindowManager : public WindowListener,
 ...
 public:
  ...
  void ChangeWindow(int src_window_id, int dst_window_id,
                    bool preview_init = false);
  void SetGUIPresenter(
      const std::map<WindowID, IGUIPresenter *> &win_presenter_map);
 ...
 private:
  ...
  static void HandlePostMessage(Window *form, int msg, int val,
                                WindowManager *self);
};
