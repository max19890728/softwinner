...
Return:
 *****************************************************************************/
int StatusBarBottomWindow::HandleMessage(HWND hwnd, int message, WPARAM wparam,
                                         LPARAM lparam) {
...
StatusBarBottomWindow::StatusBarBottomWindow(IComponent *parent)
  ...
  SetBackColor(0x66000000);  // apha b3
...
StatusBarBottomWindow::~StatusBarBottomWindow() { db_msg("destruct"); }
...
void StatusBarBottomWindow::GetPreviewButtonPos(struct buttonPos *bps,
                                                int len) {
  for (int i = 0; i < len; i++) {
...
void StatusBarBottomWindow::GetPlayBackButtonPos(struct buttonPos *bps,
                                                 int len) {
  for (int i = 0; i < len; i++) {
  ...
  if (on_off) {
    if (reload_all) {
      GraphicView::LoadImage(GetControl("button1_icon"), "preview_record",
                             "preview_record_h", GraphicView::NORMAL);
      GraphicView::LoadImage(GetControl("button2_icon"), "preview_photo",
                             "preview_photo_h", GraphicView::NORMAL);
      GraphicView::LoadImage(GetControl("button3_icon"), "preview_lock",
                             "preview_lock_h", GraphicView::NORMAL);
      GraphicView::LoadImage(GetControl("button4_icon"), "preview_playback",
                             "preview_playback_h", GraphicView::NORMAL);
      GraphicView::LoadImage(GetControl("button5_icon"), "preview_setting",
                             "preview_setting_h", GraphicView::NORMAL);
      GraphicView::LoadImage(GetControl("button6_icon"), "preview_voice",
                             "preview_voice_h", GraphicView::NORMAL);
    }
    if (flag == 0) {
...
void StatusBarBottomWindow::PreviewWindownModeButtonStatus(int flag) {
  GetControl("button2_icon")->Hide();
  if (!flag) {
    GraphicView::LoadImage(GetControl("button2_icon"), "preview_mode",
                           "preview_mode_h", GraphicView::NORMAL);
  } else {
    GraphicView::LoadImage(GetControl("button2_icon"), "preview_photo",
                           "preview_photo_h", GraphicView::NORMAL);
  }
...
void StatusBarBottomWindow::PlaybackWindownButtonStatus(bool on_off,
                                                        bool l_hilight,
                                                        bool r_hilight) {
  if (on_off) {
#ifndef USEICONTHUMB
...
#else
    GraphicView::LoadImage(GetControl("playback_button1_icon"), "pw_delete",
                           "pw_delete_h", GraphicView::NORMAL);
    GraphicView::LoadImage(GetControl("playback_button3_icon"), "pw_select",
                           "pw_select_h", GraphicView::NORMAL);
#endif
    GetControl("playback_button1_icon")->Hide();
    GetControl("playback_button2_icon")->Hide();
    GraphicView::LoadImage(GetControl("playback_button2_icon"), "pw_lock",
                           "pw_lock_y", GraphicView::NORMAL);
...
void StatusBarBottomWindow::PlayingWindownButtonStatus(bool on_off,
                                                       bool p_pause,
                                                       bool reload_all,
                                                       bool jpeg_play_flag) {
  if (on_off) {
...
#else
      GraphicView::LoadImage(GetControl("button1_icon"), "return_y",
                             "return_y_h", GraphicView::NORMAL);
      GraphicView::LoadImage(GetControl("button2_icon"), "playing_left",
                             "playing_left_h", GraphicView::NORMAL);
      GraphicView::LoadImage(GetControl("button3_icon"), "playing_right",
                             "playing_right_h", GraphicView::NORMAL);
      GraphicView::LoadImage(GetControl("button4_icon"), "delete_y",
                             "delete_y_h", GraphicView::NORMAL);
      GraphicView::LoadImage(GetControl("button5_icon"), "play", "play_h",
                             GraphicView::NORMAL);
#endif
    } else {
      if (p_pause)
...
#else
        GraphicView::LoadImage(GetControl("button5_icon"), "pause", "pause_h",
                               GraphicView::NORMAL);
#endif
      else
#ifndef USEICONTHUMB
...
void StatusBarBottomWindow::Update(MSG_TYPE msg, int p_CamID, int p_recordId) {
  ...
  switch ((int)msg) {
    ...
    case MSG_VIDEO_PLAY_COMPLETION:
      ...
      PlayingWindownButtonStatus(
          true, true, false,
          false);  // show the pause icon ,other icon not need to reload
      break;
    case MSG_PIC_PLAY_START:
      db_error("MSG_PIC_PLAY_START");
      // PlaybackWindownButtonStatus(false,false,true);//hide the playback 3
      // buttong icon
      PlayingWindownButtonStatus(
          true, true, true,
          true);  // show the first time to playing window buttong icon
      break;
    case MSG_VIDEO_PLAY_START:
      ...
      PlayingWindownButtonStatus(
          true, true, true,
          false);  // show the first time to playing window buttong icon
      break;
    ...
    case MSG_PLAYBACK_TO_PREIVEW_CHANG_STATUS_BAR_BOTTOM: {
      ...
      db_error("MSG_SETTING_TO_PREIVEW_CHANG_STATUS_BAR_BOTTOM p_CamID %d",
               p_CamID);
...
void StatusBarBottomWindow::showStatusBarBottomWindow() {
  ...
  PlaybackWindownButtonStatus(false, false,
                              true);  // hide the playback 3 buttong icon
