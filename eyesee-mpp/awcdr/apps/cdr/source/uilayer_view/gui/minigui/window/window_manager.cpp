...
void WindowManager::Init(WindowID entry_win) {
  ...
  if (entry_win_ == WINDOWID_PREVIEW) {
    ...
    PlaybackWindow *p_win =
        reinterpret_cast<PlaybackWindow *>(GetWindow(WINDOWID_PLAYBACK));
  ...
  if (entry_win_ == WINDOWID_DEBUG) {
    ...
    DebugView *debug_view = static_cast<DebugView *>(GetWindow(WINDOWID_DEBUG));
  }
...
void WindowManager::DetectSdcardSpeedAndVersion() {
  if (entry_win_ == WINDOWID_PREVIEW) {
    ...
    PreviewWindow *pre_win = reinterpret_cast<PreviewWindow *>(GetWindow(WINDOWID_PREVIEW));
