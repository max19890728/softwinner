...
buttonPos_ UsbListItemPos[2] = {
    {0, 80, 240, 80 + 80}, {0, 80 + 120, 240, 180 + 80 + 120},
};
...
std::string USBModeWindow::GetResourceName() { return std::string(GetClassName()); }
...
USBModeWindow::USBModeWindow(IComponent *parent)
  ...
  wname = "USBModeWindow";
  Load();
  std::string bkgnd_bmp = R::get()->GetImagePath("black_bg");
...
int USBModeWindow::HandleMessage(HWND hwnd, int message, WPARAM wparam,
                                 LPARAM lparam) {
  switch (message) {
    case MSG_COMMAND: {
    ...
    case MSG_LBUTTONDOWN:
      db_warn("habo--> usb mode window MSG_LBUTTONDOWN  x = %d  y = %d ",
              LOSWORD(lparam), HISWORD(lparam));
      db_error("ignore_message_flag_ is %d", ignore_message_flag_);
      if (!ignore_message_flag_) {
        ListItemClickProc(
            getTouchPosID(LOSWORD(lparam), HISWORD(lparam), UsbListItemPos, 2));
      } else {
...
void USBModeWindow::PreInitCtrl(View *ctrl, std::string &ctrl_name) {
  if (ctrl_name == std::string("usb_connect_list_title")) {
...
void USBModeWindow::OnLanguageChanged() { this->InitUSBWin(); }
