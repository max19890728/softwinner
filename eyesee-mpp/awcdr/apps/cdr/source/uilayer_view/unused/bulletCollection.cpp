...
void BulletCollection::initButtonDialog(IComponent *parent) {
  ...
  button_dialog_title =
      reinterpret_cast<TextView *>(button_dialog_->GetControl("dialog_title"));
  ...
  button_dialog_text =
      static_cast<TextView *>(button_dialog_->GetControl("info_text"));
  ...
  button_dialog_confirm = reinterpret_cast<ButtonOK *>(
      button_dialog_->GetControl("confirm_button"));
  ...
  button_dialog_cancel = reinterpret_cast<ButtonCancel *>(
      button_dialog_->GetControl("cancel_button"));
  button_dialog_cancel->SetCaption(txt_info.c_str());
  button_dialog_cancel->SetTag(CANCEL_BUTTON);

  button_dialog_progress_bar_background = reinterpret_cast<Button *>(
      button_dialog_->GetControl("dialog_progress_bar_background"));
  button_dialog_progress_bar_background->SetBackColor(0x4CD8D8D8);

  button_dialog_progress_bar_top_color = reinterpret_cast<Button *>(
      button_dialog_->GetControl("dialog_progress_bar_top_color"));
...
void BulletCollection::updateProgressbarPercentage(int value) {
  ...
  // db_msg("updateProgressbarPercentage...   ss.str().c_str() =
  // %d",ss.str().c_str());
...
void BulletCollection::HandleProgressbarTime(union sigval sigval) {
  ...
  if (100 == mpercentage) {
    ...
    if (bc->getButtonDialogCurrentId() ==
        BC_BUTTON_DIALOG_DOWNLOAD_PACKET_VERSION) {
      bc->fdownload = true;
    } else if (bc->getButtonDialogCurrentId() ==
               BC_BUTTON_DIALOG_INSTALL_PACKET_VERSION) {
      bc->finstall = true;
    }
    db_msg(
        "[debug_zhb]--------ready to dohide the button_dialog and send message "
        "SETTING_BUTTON_DIALOG");
...
void BulletCollection::buttonDialogFilling(
    const bc_button_dialog_item &_button_dialog_item_, bool button_right_show,
    bool progress_bar_show, bool m_full) {
  if (m_full)
    button_dialog_->SetPosition(320, 240, 640, 240);
  else
    button_dialog_->SetPosition(410, 240, 460, 232);
  ...
  // db_msg("[debug_zhb]--------buttonDialogFilling---info_title =
  // %s",button_dialog_str.c_str());
  ...
  // db_msg("[debug_zhb]--------buttonDialogFilling---info_text =
  // %s",button_dialog_text_str.c_str());
...
int BulletCollection::ShowButtonDialog() {
  db_msg("debug_zhb----ShowButtonDialog----m_button_dialog_flag_ = %d",
         m_button_dialog_flag_);
  ...
  switch (button_dialog_current_id_) {
    ...
    case BC_BUTTON_DIALOG_FORMAT_SDCARD:  // button are confirm and cancel ,
                                          // info_text
    case BC_BUTTON_DIALOG_PACKING_RECORD_LOOP_OPEN:  // button are open_now and
                                                     // cancel ,info_text
    case BC_BUTTON_DIALOG_4G_NETWORK_OPEN_VERSION:
    case BC_BUTTON_DIALOG_4G_NETWORK_OPEN_WATCHDOG:
    case BC_BUTTON_DIALOG_DATA_DOWNLOAD_FAIL_NETWORK_ARNORMAL:   // button are
                                                                 // restart and
                                                                 // cancel
                                                                 // ,info_text
    case BC_BUTTON_DIALOG_DATA_DOWNLOAD_FAIL_INSUFFICIENT_FLOW:  // button are
                                                                 // flow
                                                                 // recharge and
                                                                 // cancel
                                                                 // ,info_text
    ...
    case BC_BUTTON_DIALOG_DELETE_ALLSELECTED: {
      ...
      buttonDialogFilling(bc_button_dialog_item_[button_dialog_current_id_],
                          true, false);
    } break;
    case BC_BUTTON_DIALOG_DOWNLOAD_PACKET_VERSION:  // button are restart and
                                                    // cancel ,info_text
                                                    // ,progressbar
      {
      ...
      break;
    case BC_BUTTON_DIALOG_INSTALL_PACKET_VERSION:  // button is cancel
                                                   // ,info_text,progressbar
      {
      ...
      break;
    case BC_BUTTON_DIALOG_INSTALL_CURRENT_CITY_PACKET_FINISH:  // button is I
                                                               // know
                                                               // ,info_text
    case BC_BUTTON_DIALOG_INSTALL_NATIONAL_CITY_PACKET_FINISH:
    case BC_BUTTON_DIALOG_INSTALL_CURRENT_VERSION_PACKET_FINISH:
    case BC_BUTTON_DIALOG_PACKING_RECORD_LOOP_OPEN_FAIL:  // button is I know
    ...
    case BC_BUTTON_DIALOG_DD_NOTICE_FULL: {
      db_msg(
          "[debug_zhb]--------1 button  , info text , info_title  full screen");
      buttonDialogFilling(bc_button_dialog_item_[button_dialog_current_id_],
                          false, false, true);
    } break;
    default:
...
void BulletCollection::ShowButtonDialogSettingButtonStatus(int current_id) {
  ...
  db_msg(
      "[debug_zhb]------ShowButtonDialogSettingButtonStatus-------status_index "
      "= %d",
      status_index);
