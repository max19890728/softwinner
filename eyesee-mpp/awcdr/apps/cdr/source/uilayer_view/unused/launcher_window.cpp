...
int LauncherWindow::HandleMessage(HWND hwnd, int message, WPARAM wparam,
                                  LPARAM lparam) {
  switch (message) {
    case MSG_PAINT:
      return HELP_ME_OUT;
...
LauncherWindow::LauncherWindow(IComponent *parent) : SystemWindow(parent) {
  ...
  TextView *title =
      reinterpret_cast<TextView *>(dialog_->GetControl("dialog_title"));
  title->SetCaption(title_str.c_str());
  title->SetBackColor(0xFF999999);  // 灰色

  // confirm button of dialog
  Button *confirm_button =
      reinterpret_cast<Button *>(dialog_->GetControl("confirm_button"));
  ...
  Button *cancel_button =
      reinterpret_cast<Button *>(dialog_->GetControl("cancel_button"));
...
string LauncherWindow::GetResourceName() { return string(GetClassName()); }
