...
void WizardWindow::InitMenuItem(const char *item_name,
                                ...
                                const char *hilight_icon, int type, int value) {
  char path[64];
  R *r = R::get();
...
int WizardWindow::HandleMessage(HWND hwnd, int message, WPARAM wparam,
                                LPARAM lparam) {
  switch (message) {
    case MSG_PAINT:
      return HELP_ME_OUT;
...
WizardWindow::WizardWindow(IComponent *parent)
  ...
  if (!FILE_EXIST(config_file)) {
    db_warn("config file %s not exist, copy default from /usr/share/app/sdv",
            config_file);
  ...
  InitMenuItem("lv_tutk", "info_icon", "info_icon", TYPE_STRING,
               config.GetBoolValue("wizard.features.tutk"));
  InitMenuItem("lv_rtsp", "info_icon", "info_icon", TYPE_STRING,
               config.GetBoolValue("wizard.features.rtsp"));
  InitMenuItem("lv_onvif", "info_icon", "info_icon", TYPE_STRING,
               config.GetBoolValue("wizard.features.onvif"));
  ...
  TextView *title =
      reinterpret_cast<TextView *>(finish_dialog_->GetControl("dialog_title"));
  title->SetCaption(title_str.c_str());
  title->SetBackColor(0xFF999999);  // 灰色

  // confirm button of dialog
  Button *confirm_button =
      reinterpret_cast<Button *>(finish_dialog_->GetControl("confirm_button"));
  ...
  Button *cancel_button =
      reinterpret_cast<Button *>(finish_dialog_->GetControl("cancel_button"));
  ...
  title =
      reinterpret_cast<TextView *>(format_dialog_->GetControl("dialog_title"));
  title->SetCaption(title_str.c_str());
  title->SetBackColor(0xFF999999);  // 灰色

  // confirm button of dialog
  confirm_button =
      reinterpret_cast<Button *>(format_dialog_->GetControl("confirm_button"));
  confirm_button->SetTag(CONFIRM_BUTTON);
  confirm_button->OnPushed.bind(this, &WizardWindow::FormatDialogProc);

  // // cancel button of dialog
  cancel_button =
      reinterpret_cast<Button *>(format_dialog_->GetControl("cancel_button"));
  cancel_button->SetTag(CANCEL_BUTTON);
  ...
  title =
      reinterpret_cast<TextView *>(info_dialog_->GetControl("dialog_title"));
  title->SetCaption(title_str.c_str());
  title->SetBackColor(0xFF999999);  // 灰色

  TextView *info_text =
      static_cast<TextView *>(info_dialog_->GetControl("info_text"));
...
string WizardWindow::GetResourceName() { return string(GetClassName()); }
