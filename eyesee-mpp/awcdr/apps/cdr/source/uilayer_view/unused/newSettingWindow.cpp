...
void NewSettingWindow::InitListviewbmpMap()
{
	...
	BITMAP* pbmp;
	std::string ss;
	for (index = 0; index < SETTING_LISTVIEW_NUM; index++) {
		iconPath = ListBoxItemData[index].first_icon_path;
		if (iconPath) {
			pbmp = AllocListViewImage((const char*)((r->GetImagePath(iconPath)).c_str()));
			ss = iconPath;
			listviewbmp.insert(make_pair(ss, pbmp));
		}
		iconPath = ListBoxItemData[index].second_icon_path0;
		if (iconPath) {
			pbmp = AllocListViewImage((const char*)((r->GetImagePath(iconPath)).c_str()));
			ss = iconPath;
			listviewbmp.insert(make_pair(ss, pbmp));
		}
		iconPath = ListBoxItemData[index].second_icon_path1;
		if (iconPath) {
			pbmp = AllocListViewImage((const char*)((r->GetImagePath(iconPath)).c_str()));
			ss = iconPath;
			listviewbmp.insert(make_pair(ss, pbmp));
	...
	for (index = 0; index < SETTING_LISTVIEW_NUM_PHOTO; index++) {
		iconPath = ListBoxItemDataPhoto[index].first_icon_path;
		if (iconPath) {
			pbmp = AllocListViewImage((const char*)((r->GetImagePath(iconPath)).c_str()));
			ss = iconPath;
			listviewbmp.insert(make_pair(ss, pbmp));
		}
		iconPath = ListBoxItemDataPhoto[index].second_icon_path0;
		if (iconPath) {
			pbmp = AllocListViewImage((const char*)((r->GetImagePath(iconPath)).c_str()));
			ss = iconPath;
			listviewbmp.insert(make_pair(ss, pbmp));
		}
		iconPath = ListBoxItemDataPhoto[index].second_icon_path1;
		if (iconPath) {
			pbmp = AllocListViewImage((const char*)((r->GetImagePath(iconPath)).c_str()));
			ss = iconPath;
			listviewbmp.insert(make_pair(ss, pbmp));
	...
	db_info("listviewbmp size: %d",listviewbmp.size());
}
...
void NewSettingWindow::InitListViewItem()
{
	...
	db_error("Check Point: %d %d", winstatus, STATU_PREVIEW);
	if (winstatus == STATU_PREVIEW) 
	{
		...
		db_error("Check Point: menucount %d", menucount);
		if (menucount == SETTING_LISTVIEW_NUM) {
			db_error("Check Point: Test1");
	...
	db_error("Check Point: InitListViewItem done.");
}
			...
			const int  type, const char *second_icon_path0, const char *second_icon_path1,int index_)
{
	...
	db_error("Check Point: index %d", index_);
	...
	db_error("Check Point: first text %s", first_text);
	...
	//db_error("Check Point: info text %s", info_str.str().c_str());
	...
	db_error("Check Point: image path %s", ss);
...
void NewSettingWindow::ShowSubList(int index_msg)
{
	...
	item_sub.nItemHeight = (rect.bottom - rect.top) / 3;
	LVSUBITEM subdata;
	StringVector sub_value_list;
