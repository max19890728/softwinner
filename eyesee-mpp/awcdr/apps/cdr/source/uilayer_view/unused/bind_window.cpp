...
void BindWindow::keyProc(int keyCode, int isLongPress) {
  switch (keyCode) {
    ...
    case SDV_KEY_OK: {
      DoHide();
      // modify by zhb 2018.09.12
      auto *pre_win = reinterpret_cast<PreviewWindow *>(
...
#if 0
      auto &partition_manager = PartitionManager::GetInstance();
      partition_manager->sunxi_spinor_private_sec_set(FKEY_BINDFLAG, "true");
      partition_manager->sunxi_spinor_private_set_flag(1);
#endif
    } break;
    case SDV_KEY_RIGHT:
      break;
...
int BindWindow::HandleMessage(HWND hwnd, int message, WPARAM wparam,
                              LPARAM lparam) {
  // db_msg("[debug_jaosn]:BindWindow message = %d,wparam = %u,lparam =
  ...
  switch (message) {
    case MSG_PAINT: {
      ...
      if (access("/data/BindImage.png", R_OK) == 0) {
        ...
        if (qrbmp != NULL) FillBoxWithBitmap(hdc, 64, 149, 243, 243, qrbmp);
      }
      EndPaint(hwnd, hdc);
...
BindWindow::BindWindow(IComponent *parent) : SystemWindow(parent), qrbmp(NULL) {
  wname = "BindWindow";
  Load();
  db_msg("");
  ...
  auto *bw_title_label =
      reinterpret_cast<TextView *>(GetControl("title_label"));
  ...
  auto *bw_text_label = reinterpret_cast<TextView *>(GetControl("text_label"));
...
BindWindow::~BindWindow() {
  ...
  if (qrbmp != NULL) UnloadBitmap(qrbmp);
}
...
string BindWindow::GetResourceName() { return string(GetClassName()); }
...
void BindWindow::PreInitCtrl(View *ctrl, string &ctrl_name) {
  if (ctrl_name == "text_label" || ctrl_name == "title_label") {
...
void BindWindow::DoShow() { Window::DoShow(); }

void BindWindow::DoHide() { SetVisible(false); }
...
void BindWindow::DoHideBindWindow() {
  PreviewWindow *pre_win =
      reinterpret_cast<PreviewWindow *>(WindowManager::GetInstance()->GetWindow(
          WINDOWID_PREVIEW));  // modify by zhb 2018.04.17
  if (pre_win->DetectSdcardNewVersion() < 0)
    pre_win->keyProc(SDV_KEY_OK, SHORT_PRESS);
  listener_->sendmsg(this, WM_WINDOW_CHANGE,
                     WINDOWID_PREVIEW);  // WINDOWID_SETTING
}
