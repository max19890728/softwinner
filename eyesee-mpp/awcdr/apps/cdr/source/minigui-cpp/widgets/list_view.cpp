...
ListView::ListView(View *parent) : SystemWidget(parent) {
  isKeyUp = true;
  downKey = 0;
  itemCont = 0;
  hi_idx_ = -1;
...
void ListView::keyProc(int keyCode, int isLongPress) {
  switch (keyCode) {
    case SDV_KEY_LEFT: {
      int ret = 0;
      db_warn("[debug_jaosn]:SDV_KEY_LEFT GetItemCont() = %d ,itemCont = %d",
              GetItemCont(), itemCont);
...
int ListView::GetCurItem() { return itemCont; }

int ListView::SetCurItem(int curitem) { itemCont = curitem; }

int ListView::HandleMessage(HWND hwnd, int message, WPARAM wparam,
                            LPARAM lparam) {
  switch (message) {
#ifdef SETBG
    case MSG_ERASEBKGND: {
      ...
      if (bg_image_ != NULL) {
        db_msg(
            "zhb---------retemp.left = %d  retemp.right = %d   RECTW(rcTemp) = "
            "%d    RECTH(rcTemp) = %d",
            rcTemp.left, rcTemp.right, RECTW(rcTemp), RECTH(rcTemp));
        FillBoxWithBitmap(hdc, 0, 0, RECTW(rcTemp), RECTH(rcTemp), bg_image_);
      } else {  // <C7><E5><B3><FD><CE><DE>Ч<C7><F8><D3><F2>,
                // <C8><E7><B9><FB>û<D3><D0>Ϊ<B4><B0><BF><DA><C9><E8><D6>ñ<B3><BE><B0>ͼƬ<A3><AC><D4><F2><D2><D4><U+0378><C3><U+1F1CFE><B0><CC><EE><B3><E4>
...
int ListView::CancleAllHilight() { return 0; }
...
int ListView::GetSelectedItem(HLVITEM &item_handle, LVITEM &lvitem) {
  item_handle =
      (HLVITEM)SendMessage(handle_, LVM_GETSELECTEDITEM, 0, (LPARAM)&lvitem);
...
string ListView::GetItemText(HLVITEM item, int col) {
  ...
  ret = SendMessage(handle_, LVM_GETSUBITEMTEXT, (WPARAM)item,
                    (LPARAM)&item_data);
...
int ListView::GetHilight() { return hi_idx_; }
