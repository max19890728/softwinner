...
GraphicView::GraphicView(View *parent)
    : SystemWidget(parent), auto_hi_(true), button_mode_(true), timeout_(50) {
...
GraphicView::~GraphicView() {
  ...
  if (highlight_image_.bmBits) UnloadBitmap(&highlight_image_);
}
...
void GraphicView::LoadImageFromAbsolutePath(View *ctrl,
                                            const std::string &path) {
  GraphicView *view = reinterpret_cast<GraphicView *>(ctrl);
  if (view) view->SetImage(path);
}

void GraphicView::LoadImage(View *ctrl, const char *normal,
                            const char *highlight,
                            enum GraphicViewState state) {
  GraphicView *view = reinterpret_cast<GraphicView *>(ctrl);
  if (view) {
    if (view->normal_image_.bmBits) UnloadBitmap(&view->normal_image_);
    if (view->highlight_image_.bmBits) UnloadBitmap(&view->highlight_image_);
    std::string normal_path = R::get()->GetImagePath(normal);
    ::LoadBitmapFromFile(HDC_SCREEN, &(view->normal_image_),
                         normal_path.c_str());
    std::string highlight_path = R::get()->GetImagePath(highlight);
    ::LoadBitmapFromFile(HDC_SCREEN, &(view->highlight_image_),
...
void GraphicView::GetCreateParams(CommonCreateParams &params) {
  ...
  params.style =
      WS_VISIBLE | SS_NOTIFY | SS_BITMAP | SS_CENTERIMAGE | option_style_;
...
enum GraphicView::GraphicViewState GraphicView::GetState() { return state_; }

void GraphicView::AutoHightLight(bool enable) { auto_hi_ = enable; }

void GraphicView::SetButtonMode(bool enable) { button_mode_ = enable; }

void GraphicView::SetTimeOut(int msec) { timeout_ = msec / 10; }
