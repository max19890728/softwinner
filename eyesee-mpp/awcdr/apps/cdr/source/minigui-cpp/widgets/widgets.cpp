...
Widget::Widget(View *parent)
    : View(parent), handle_(HWND_INVALID), option_style_(0) {}

Widget::~Widget() { DestroyWidget(); }
...
void Widget::SetOptionStyle(const DWORD &style) { option_style_ = style; }

void Widget::GetOptionStyle(DWORD &style) { style = option_style_; }
...
*****************************************************************************/
long int Widget::WindowProc(HWND hwnd, unsigned int message, WPARAM wparam,
                            LPARAM lparam) {
  Widget *widget = NULL;
  int ret = 0;
...
/*****************************************************************************
 ...
 Return: -
*****************************************************************************/
void Widget::Refresh() { ::InvalidateRect(GetHandle(), 0, TRUE); }
...
void Widget::SetVisible(bool new_val) {
  if (new_val) {
    if (!parent_ || parent_->GetVisible()) Show();
  } else {
    Hide();
  }
}
...
void Widget::CreateWidget() {
  ...
  handle_ =
      CreateWindowEx(params.class_name, "", WS_CHILD | params.style,
                     WS_EX_NONE | params.exstyle, params.id, params.x, params.y,
                     params.w, params.h, parent_handle, (DWORD) this);
...
CustomWidget::CustomWidget(View *parent) : Widget(parent) {}
