...
SystemWidget::SystemWidget(View *parent) : Widget(parent) {}

SystemWidget::~SystemWidget() { DestroyWidget(); }
...
/*****************************************************************************
 ...
 Return:
*****************************************************************************/
long int SystemWidget::WindowProc(HWND hwnd, unsigned int message,
...
void SystemWidget::CreateWidget() {
  ...
  handle_ = ::CreateWindowEx(params.class_name, " ", WS_CHILD | params.style,
                             WS_EX_NONE | params.exstyle, params.id, params.x,
                             params.y, params.w, params.h, parent_handle, 0);
  old_window_proc_ = SetWindowCallbackProc(handle_, WindowProc);
  db_msg("create system widget: %s, %s, %p", params.alias, params.class_name,
         handle_);
