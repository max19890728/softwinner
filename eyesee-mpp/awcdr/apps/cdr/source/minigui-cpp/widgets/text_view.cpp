...
int TextView::HandleMessage(HWND hwnd, int message, WPARAM wparam,
  ...
  switch (message) {
    case MSG_PAINT: {
      ...
      db_debug("draw in rect{%d, %d, %d, %d}", text_rect.left, text_rect.top,
...
void TextView::GetText(string &text) { text = text_; }
...
// 调用SetCaption会概率性出现对话框字符串没有显示出来
// 直接调用minigui绘制字符串函数来实现
void TextView::SetCaptionEx(const char *new_caption) {
  text_ = new_caption;
...
void TextView::SetCaptionColor(DWORD new_color) { text_color_ = new_color; }

void TextView::SetTextStyle(uint32_t style) { text_style_ = style; }

int TextView::OnMouseUp(unsigned int button_status, int x, int y) {
  if (TextOnClick) TextOnClick(this);
  return HELP_ME_OUT;
}
