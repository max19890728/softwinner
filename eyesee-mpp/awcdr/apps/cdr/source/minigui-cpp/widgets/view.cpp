...
int View::OnMouseUp(unsigned int button_status, int x, int y) {
  if (view_state_ == STATE_CLICKED) {
    ...
    ::PostMessage(GetHandle(), VIEW_MOUSE_CLICK, button_status, MAKELONG(x, y));
  }
  return HELP_ME_OUT;
}
...
/*****************************************************************************
 ...
 Return:
    HELP_ME_OUT - DefaultMainWinProc process
*****************************************************************************/
int View::HandleMessage(HWND hwnd, int message, WPARAM wparam, LPARAM lparam) {
  switch (message) {
...
void View::SetPosition(PRECT new_rect) { Resize(new_rect); }

void View::Resize(const PRECT new_rect) {
  NormalizeRect(new_rect);
  if (!EqualRect(&bound_rect_, new_rect)) {
    db_debug("Resize to {%d, %d, %d, %d}", new_rect->left, new_rect->top,
             new_rect->right, new_rect->bottom);
    bound_rect_ = *new_rect;
    ::MoveWindow(GetHandle(), new_rect->left, new_rect->top, RECTW((*new_rect)),
                 RECTH((*new_rect)), true);
  }
}
...
/*****************************************************************************
 ...
 Return:
*****************************************************************************/
void View::GetRect(const PRECT new_rect) { *new_rect = bound_rect_; }

HWND View::GetHandle() { return HWND_NULL; }
...
bool View::GetVisible() { return is_visible_; }

void View::Show() {
  if (is_visible_ != true) is_visible_ = true;
}

void View::Hide() {
  if (is_visible_ != false) is_visible_ = false;
}

DWORD View::GetBackColor() { return back_color_; }

void View::SetBackColor(DWORD new_value) { back_color_ = new_value; }
...
/*****************************************************************************
 ...
 Return:
*****************************************************************************/
void View::SetTag(int new_value) { tag_ = new_value; }

int View::GetTag() { return tag_; }
...
void View::SetCtrlStyle(DWORD style_) { transparent_style_ = style_; }

void View::Refresh() { ::InvalidateRect(GetHandle(), 0, TRUE); }
