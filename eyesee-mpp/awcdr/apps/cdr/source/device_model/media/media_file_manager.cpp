      ...
      db_start_up_init_(false)
{
    ...
    pthread_mutex_init(&m_db_lock, nullptr);
...
MediaFileManager::~MediaFileManager()
{
    ...
    if (DB_updata_thread_id > 0) pthread_detach(DB_updata_thread_id);
    if (DB_init_thread_id > 0) pthread_detach(DB_init_thread_id);
    sm->Detach(this);
...
int MediaFileManager::startCreateDataBase()
{
    ...
    if ((ret != UMOUNT) && (ret != STORAGE_FS_ERROR) && (ret != FORMATTING)) {
        if (DB_updata_thread_id > 0) pthread_detach(DB_updata_thread_id);
        if (DB_init_thread_id > 0) pthread_detach(DB_init_thread_id);
        if (DB_init_thread_id == 0) ThreadCreate(&DB_init_thread_id, nullptr, MediaFileManager::DBInitThread, this);
...
int8_t MediaFileManager::DBInit()
{
    ...
    if ((storageStatus == UMOUNT) || (storageStatus == STORAGE_FS_ERROR) || (storageStatus == FORMATTING)) {
        m_IgnoreScan = false;
        return 0;
    }
    bool reCreateDb_A = false;
    if (db_inited_) goto out;
    ...
    try {
        ...
        if (access(DB_PATH, F_OK) < 0) {
            ...
            if (access("/mnt/extsd/.tmp/sqlite/sunxi.db", F_OK) < 0) reCreateDb_A = true;
        }
        if (NULL == db_) {
            ...
            if (db_ == NULL) {
                ...
                db_info("create sunxi.db successed");
            }
        } else {
            db_info("sunxi.db is exist");
        }
    ...
    exec:
        if (db_) {
            if (reCreateDb_A) {
                ...
                db_->exec("CREATE TABLE " DATA_TABLE
                          " (file VARCHAR(128) PRIMARY KEY, time INTEGER, type VARCHAR(32), info VARCHAR(32), size "
                          "INTEGER, Lock INTEGER, LockTime INTEGER, checkstate INTEGER, keyString VARCHAR(128), "
                          "orderNum VARCHAR(32))");
            }
            db_new_create_ = true;
        }
...
int8_t MediaFileManager::DBReset()
{
    ...
    if (DB_init_thread_id == 0) {
        pthread_create(&DB_init_thread_id, nullptr, MediaFileManager::DBInitThread, this);
        pthread_join(DB_init_thread_id, nullptr);
    }
    return 0;
}

uint32_t MediaFileManager::GetMediaFileCnt(const std::string &media_type, int p_CamId)
{
...
int8_t MediaFileManager::GetMediaFileList(std::vector<std::string> &file_list, uint32_t idx_s, uint16_t list_cnt,
                                          bool order, const std::string &media_type, int p_CamId)
{
    std::string file_name;
    std::string sort_order = order ? "asc" : "desc";
    std::string filter;
    pthread_mutex_lock(&m_db_lock);
    ...
    pthread_mutex_unlock(&m_db_lock);
    if (!media_type.empty()) filter = "where type='" + media_type + "' ";
...
const std::string MediaFileManager::GetMediaFileType(const std::string &file_name, int p_CamId)
{
    std::string filter;
...
int8_t MediaFileManager::DeleteFileByName(const std::string &file_name, int p_CamId)
{
    int ret = 0;
    int ret1 = 0;
    int ret2 = 0;
    ...
    if (p_CamId == 0 || 1 == p_CamId) {
        std::string thumb_pic = GetThumbPicName(file_name, p_CamId);
        sm->RemoveFile(thumb_pic.c_str());
    }
    std::string filter = "WHERE file='" + file_name + "'";
...
int8_t MediaFileManager::DeleteFilesByType(const std::string &media_type, int p_CamId)
{
    std::string filter;
    StorageManager *sm = StorageManager::GetInstance();
    if (0 == p_CamId)
...
int8_t MediaFileManager::DeleteLastFilesDatabaseIndex(const std::string &file_name, int p_CamId)
{
    int ret = 0;
    int ret1 = 0;
    int ret2 = 0;
    ...
    try {
        if (0 == p_CamId || 1 == p_CamId) {
            ...
            if (query.executeStep()) {
                ...
                // FIXME: 實際上 int ret 永遠無法為 NULL?
                ret = static_cast<int>(NULL);  // no media_type you want to find
            }
        }
    } catch (SQLite::Exception e) {
        db_error("SQLITE ERROR: %s", e.what());
...
int8_t MediaFileManager::DeleteFileByOrder(const std::string &media_type, bool order, int p_CamId, bool m_force)
{
    int ret = -1;
    std::string sort;
    std::string filter;
...
int8_t MediaFileManager::DeleteLastFile(const std::string &media_type, int cnt, int p_CamId, bool m_force)
{
    int ret = -1;
    for (int i = 0; i < cnt; i++) ret = this->DeleteFileByOrder(media_type, false, p_CamId, m_force);
    return ret;
}
...
int8_t MediaFileManager::AddFile(const MediaFile &file, int p_CamId, int64_t keyValue, bool lock_flag)
{
    ...
    std::string OrderId;
    OrderId.clear();
    if (!m_orderId.empty()) OrderId = m_orderId;
    pthread_mutex_lock(&m_db_lock);
    pthread_mutex_lock(&m_lock);
    if (db_ == NULL) {
        db_warn("data base is null! db_: db_inited:%d", db_, db_inited_);
...
int8_t MediaFileManager::AddFile(const MediaFile &file, SQLite::Statement &stm, int count, int p_CamId,
                                 int64_t keyValue, std::string order_Num, bool lock_flag)
{
    int ret = -1;
    int total_cnt = 0;
    ...
    try {
        ...
        uint64_t fileSize = (uint64_t)file.GetFileSize();
        // unsigned long long fileSize = (unsigned long long)file.GetFileSize();
        stm.reset();
        if (lock_flag && ((file.GetMediaType() == VIDEO_A) || (file.GetMediaType() == VIDEO_B))) {  // 加锁的
            ...
            if (index == -1) {
                ...
                index = s1.find(".");
#ifdef VIDEOTYPE_MP4
                // (size_t pos, size_t len, const string& str) 用str替换指定字符串从起始位置pos开始长度为len的字符
                s2 = s1.replace(index, 4, "_SOS.mp4");
#else
                s2 = s1.replace(index, 3, "_SOS.ts");
#endif
                m_lockFileName = s2;
                // /mnt/extsd/event/20190718_135403_SOS.mp4
                db_warn("after replace the video file name is %s", s2.c_str());
            }
            stm.bind(1, s2.c_str());
            filetoadd = s2;
            // TODO(uknow): rename video ths pic file
            s = file.GetVideoThumbPicFileName();
            // /mnt/extsd/video/20190718_135403_ths.jpg
            db_warn("before replace the video thumb pic file name is %s", s.c_str());
            index = s.find("video");
            if (index == -1) {
                db_warn("can't find thumb pic video string");
            } else {
                s1 = s.replace(index, 5, "event");
                // /mnt/extsd/event/20190718_135403_ths.jpg
                db_warn("after replace the video thumb pic file name  is %s", s1.c_str());
                index = s1.find("_ths");
                s2 = s1.replace(index, 12, "_SOS_ths.jpg");
                m_lockthumbPicFileName = s2;
                // /mnt/extsd/event/20190718_135403_SOS_ths.jpg
                db_warn("after replace the video thumb pic file name  is %s", s2.c_str());
                media_ths_file = s2;  // /mnt/extsd/event/20190718_131805_ths_SOS.jpg
            }
        } else {  // 普通/park,  开机扫描媒体文件
            // db_error("nor / park");
        ...
        stm.bind(2, (const int64_t)file.GetFileTime());
        stm.bind(3, media_type);
        stm.bind(4, info);
        stm.bind(5, (const int64_t)fileSize);
        stm.bind(6, (const int64_t)0);
        stm.bind(7, (const int64_t)0);
        stm.bind(8, (const int64_t)0);
        stm.bind(9, (const int64_t)keyValue);  // fallocate size
        stm.bind(10, order_Num);                 // order number
        ret = stm.exec();
        // db_warn("add file to db: %s",  filetoadd.c_str());
    } catch (SQLite::Exception e) {
        db_error("SQLITE ERROR: %s", e.what());
        if (std::string(e.what()).find("UNIQUE") >= 0) {
            return 0;
        }
        return -1;
    }
    ...
    FileInfo.file_size = static_cast<int>(file.GetFileSize());
...
static int photo_file_filter(const struct dirent *dir)
{
    ...
    if (!S_ISDIR(buf.st_mode)) {
#ifdef USE_CAMB
        if (fnmatch("[0-9]*_[0-9]*[FR].jpg", dir->d_name, FNM_CASEFOLD) == 0) return 1;
#else
...
uint32_t MediaFileManager::getFilefallocateSize(const std::string p_DirName)
{
    ...
    std::ifstream fin{"/tmp/fallocate.txt"};
    if (!fin) {
        db_error("some this is wrong, fin filed");
    } else {
        std::string fallocatesize;
        int64_t size = 0;
        while (std::getline(fin, fallocatesize)) {
...
{
    struct dirent **namelist = reinterpret_cast<dirent **>(NULL);
    int ret;
    if (p_IsVideo)
    ...
    if (ret < 0) {
        ...
        if (getNewDbCreateFlag(P_CamID)) {
            for (int i = 0; i < ret; i++) {
                ...
                std::string full_name = p_scanPath + namelist[i]->d_name;
                if (P_CamID == 0) {  // ignore recording file
                    ...
                    if (rec != NULL) {
                        if (rec->RecorderIsBusy()) {
                            ...
                            if (full_name == rec_fileName) continue;
                        }
                    }
                    #endif
                }
                if (IsFileExist(full_name, P_CamID, 1) != true) {
                    SQLite::Statement stm(*self->getDBHandle(P_CamID),
...
int8_t MediaFileManager::DBUpdate()
{
    if (DB_updata_thread_id == 0) {
        ...
        pthread_create(&DB_updata_thread_id, nullptr, MediaFileManager::DoDBUpdate, this);
        pthread_join(DB_updata_thread_id, nullptr);
    }
    return 0;
}

int8_t MediaFileManager::RemoveFile(const std::string &filename) { return this->DeleteFileByName(filename); }
...
std::string MediaFileManager::GetThumbPicName(const std::string &file_name, int p_CamId)
{
    std::string result_alias_bak;
    std::string result_alias;
    result_alias_bak = file_name.c_str();
    std::string::size_type rc = result_alias_bak.rfind(".");
    if (rc == std::string::npos) {
        db_warn("invalid fileName:%s", result_alias_bak.c_str());
        return "";
    }
    std::string type = MediaFileManager::GetInstance()->GetMediaFileType(file_name, p_CamId);
...
std::string MediaFileManager::GetThumbVideoName(const std::string &file_name)
{
    std::string result_alias_bak;
    std::string result_alias;
    result_alias_bak = file_name.c_str();
...
bool MediaFileManager::IsFileExist(const std::string &p_FileName, int p_CamId, int p_CountCapacity)
{
    std::string filter;
    std::string file_name;
    bool ret = true;
        ...
        if (0 == p_CamId || 1 == p_CamId) {
            ...
            if (query.executeStep()) {
                ...
                if (p_CountCapacity) {
                    ...
                    if (!FileName.empty()) strncpy(FileInfo.file_name, FileName.c_str(), sizeof(FileInfo.file_name));
                    if (!FilePath.empty()) strncpy(FileInfo.file_path, FilePath.c_str(), sizeof(FileInfo.file_path));
...
int MediaFileManager::SetFileLockStatus(const std::string &p_FileName, int p_LockStatus, int p_CamId)
{
    std::string filter;
    std::string file_name;
    char buf[64] = {0};
    ...
    if (p_LockStatus == LockStatus) return 0;
    pthread_mutex_lock(&m_db_lock);
    if (db_ == NULL || db_inited_ == false) {
        db_warn("data base is null! db_: %p, db_inited: %d", db_, db_inited_);
        pthread_mutex_unlock(&m_db_lock);
...
int MediaFileManager::GetFileLockStatus(const std::string &p_FileName, int *p_LockStatus, int p_CamId)
{
    std::string filter;
    std::string file_name;
    int ret = -1;
...
void MediaFileManager::timeString2Time_t(const std::string &timestr, time_t *time)
{
...
int MediaFileManager::GetFileList(const std::string p_startTime, const std::string p_stopTime,
                                  std::vector<std::string> &p_FileNameList, int p_CamId, std::string p_OrderId)
{
    ...
    char buf[128] = {0};
    p_startTime_ = (unsigned int)strtol(p_startTime.c_str(), nullptr, 10);
    p_stopTime_ = (unsigned int)strtol(p_stopTime.c_str(), nullptr, 10);
                ...
                if (col.isText()) {
                    ...
                    if (!file_name.empty()) {
                        if (p_OrderId.empty()) {
                            ...
                            if (col_order.getString() == p_OrderId) p_FileNameList.push_back(file_name);
                        }
                    }
                } else {
                    db_error("get wrong data type");
...
int MediaFileManager::setFileLockTime(const std::string p_FileName, const std::string p_LockTime, int p_CamId,
                                      int p_action)
{
    std::string filter;
    std::string file_name;
    ...
    pthread_mutex_unlock(&m_db_lock);
    snprintf(buf, sizeof(buf), "SET LockTime=%ld", (int32_t)strtol(p_LockTime.c_str(), nullptr, 10));
    filter = "where file='" + p_FileName + "'";
    std::string sql = "UPDATE ";
    sql.append(DATA_TABLE);
    sql.append(" ");
...
int MediaFileManager::InsertLockInfoToMap(std::map<std::string, std::string> &p_Info, const std::string &file_name,
                                          const std::string &LockTime)
{
    ...
    if (it != p_Info.end()) {
        ...
        p_Info.insert(std::make_pair(file_name.c_str(), LockTime.c_str()));
        return 1;
    }
    return 0;
}
        ...
        if (0 == p_CamId || 1 == p_CamId) {
            ...
            while (query.executeStep()) {
                ...
                if (col.isText()) {
                    ...
                    file_name = col.getString();
                    if (!file_name.empty())
                        InsertLockInfoToMap(p_Info, file_name, std::to_string((uint64_t)LockTime));
                }
            }
        }
    } catch (SQLite::Exception e) {
...
int MediaFileManager::getLockFileCount(int p_CamId)
{
    ...
    try {
        if (0 == p_CamId || 1 == p_CamId) {
            SQLite::Statement query(*db_, sql);
            while (query.executeStep()) mCount++;
        }
    } catch (SQLite::Exception e) {
        db_error("SQLITE ERROR: %s", e.what());
    }
...
{
    int ret = 0;
    std::string filter = "WHERE checkstate=0";
    std::string sql = "DELETE FROM ";
    sql.append(DATA_TABLE);
    sql.append(" ");
    sql.append(filter);
...
int8_t MediaFileManager::clearCheckFlag(int P_CamID)
{
    ...
    pthread_mutex_unlock(&m_db_lock);
    std::string filter = "SET checkstate=0";
    std::string sql = "UPDATE ";
    sql.append(DATA_TABLE);
    sql.append(" ");
    sql.append(filter);
...
int MediaFileManager::GetFileKey(const std::string &p_FileName, std::string &keystring)
{
    std::string filter;
    std::string file_name;
    int ret = -1;
...
int MediaFileManager::GetDeleteFileList(std::map<std::string, std::string> &file_list, int p_CamId)
{
    int count = 0;
    std::string file_name;
    std::string filter = " order by time asc ";
            ...
            while (query.executeStep()) {
                ...
                if (col.isText()) {
                    ...
                    if (!file_name.empty()) {
                        ...
                        count++;
                        if (count > 3) break;
                        if (query.getColumn(5).getInt()) InsertLockInfoToMap(file_list, file_name, unlock_time);
                    }
                }
            }
        }
        ...
        if (0 == p_CamId || 1 == p_CamId) {
            ...
            while (query.executeStep()) {
                ...
                if (col.isText()) {
                    ret = 0;
                    file_name = col.getString();
                    if (!file_name.empty()) InsertLockInfoToMap(file_list, file_name, keystring);
                }
            }
        }
    } catch (SQLite::Exception e) {
            ...
            while (query.executeStep()) {
                ...
                if (col.isText()) {
                    ...
                    if (!file_name.empty()) {
                        ...
                        fileinfo.time_value = (const int64_t)query.getColumn(1).getInt64();
                        fileinfo.typeStr = query.getColumn(2).getString();
                        fileinfo.infoStr = query.getColumn(3).getString();
                        fileinfo.sizeValue = (const int64_t)query.getColumn(4).getInt64();
                        fileinfo.LockValue = (const int64_t)query.getColumn(5).getInt64();
                        fileinfo.LockTimeValue = (const int64_t)query.getColumn(6).getInt64();
                        fileinfo.CheckStateValue = (const int64_t)query.getColumn(7).getInt64();
...
{
    std::string::size_type rc = p_FileName.rfind("/");
    if (rc == std::string::npos) return "";
    return p_FileName.substr(rc + 1);
}

std::string MediaFileManager::getFilePath(std::string p_FilePath)
{
    std::string::size_type rc = p_FilePath.rfind("/");
    if (rc == std::string::npos) return "";
...
{
    if (!fileType.empty()) {
        if (strstr(fileType.c_str(), "photo") != NULL) return 0;
        if (strstr(fileType.c_str(), "video") != NULL) return 1;
        if (strstr(fileType.c_str(), "audio") != NULL) return 2;
    }
    return 1;
}

int MediaFileManager::SetFileInfoByName(const std::string &fileName, const std::string &fileName_dts,
                                        const std::string typeStr, int lockStatus, int p_CamId)
{
    int ret = -1;
    char buf[256] = {0};
...
int MediaFileManager::GetThumbFromMainPic(const std::string &src_pic, const std::string &dest_pic)
{
    ...
    if (src_data_size != 0) {
        ...
        db_msg("jpeg_size %d thumb_offset %d jpeg_thumb_len %d", jpeg_size, jpeg_thumb_offset, jpeg_thumb_len);
