...
int WavParser::GetPcmParam(FILE *file, PcmWaveParam &param) {
  ...
  // db_info("audio file format(%d, %d, %d)", param.trackCnt, param.bitWidth,
  //         param.sampleRate);
...
int WavParser::ParseHeader(FILE *file, WaveHeader &header) {
  ...
  if ((ret = strncmp("WAVE", (char *)&header.riff_fmt, 4))) {
    ...
    db_error("audio file is not wav file! exit Parser!(%d)(%s)(%c%c%c%c)", ret,
             "WAVE", ptr[0], ptr[1], ptr[2], ptr[3]);
    return -1;
  }
...
WavPlayer::WavPlayer()
    : ao_dev_(0), ao_chn_(0), callback_cnt_(0), chunk_cnt_(0) {
...
int WavPlayer::AOChannelInit(const PcmWaveParam &param) {
  ...
  // db_debug(
  //     "enWorkmode:%d enSoundmode:%d u32EXFlag:%d u32FrmNum:%d "
  //     "u32PtNumPerFrm:%d u32ChnCnt:%d u32ClkSel:%d",
  //     mAIOAttr.enWorkmode, mAIOAttr.enSoundmode, mAIOAttr.u32EXFlag,
  //     mAIOAttr.u32FrmNum, mAIOAttr.u32PtNumPerFrm, mAIOAttr.u32ChnCnt,
  //     mAIOAttr.u32ClkSel);
  ...
  // db_debug("AIO_ATTR_S: [%d, %d, %d]", mAIOAttr.u32ChnCnt,
  //          mAIOAttr.enSamplerate, mAIOAttr.enBitwidth);
...
int WavPlayer::Play(const PcmWaveData &pcm_data) {
  ...
  // db_info("pcmSize:%d, chunkSize: %d, chunkTotalCnt:%d", pcm_data.size,
  // chunk_size, chunk_cnt_);
...
ERRORTYPE WavPlayer::MPPAOCallback(void *cookie, MPP_CHN_S *pChn,
                                   MPP_EVENT_TYPE event, void *pEventData) {
  WavPlayer *self = (WavPlayer *)cookie;

  if (event == MPP_EVENT_RELEASE_AUDIO_BUFFER) {
    self->callback_cnt_++;
    // db_debug("notify release data! callback_cnt_: %d, chunk_cnt_: %d",
    // self->callback_cnt_, self->chunk_cnt_);
    if (self->callback_cnt_ == self->chunk_cnt_) {
      // db_debug("AO callback_cnt_:%d, find EOF, pcm play END!",
      //          self->callback_cnt_);
