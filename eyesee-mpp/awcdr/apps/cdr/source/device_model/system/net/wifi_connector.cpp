...
int WifiController::WifiStaConf(const std::string &ssid, const std::string &pwd)	//max+
{
	int ret = 0;
	ret = wifi_sta_conf(ssid.c_str(), pwd.c_str());
	if (ret) {
        db_msg ("Do wifi_sta_conf fail! ret:%d", ret);
        return -1;
    }
	return 0;
}
