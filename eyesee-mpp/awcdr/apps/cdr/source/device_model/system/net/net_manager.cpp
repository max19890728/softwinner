...
int NetManager::SwitchToSoftAp(const char *ssid, const char *pwd, int ch, int mode, int encrypt, int freq)
{
    ...
    ret = this->SetNetDevIp(WIFI_INTERFACE_NAME, "192.168.43.1");
    if (ret < 0) {
...
int NetManager::EnableMonitorMode(const string &interface, const string &ssid, const string &pwd)
{
    int ret = 0;
	...
	if (wifi_ctrl_->WifiStaConf(ssid, pwd) < 0) {
        db_error("WifiStaConf failed");
        return -1;
    }
