...
int PowerManager::getAhdInsertOnline(int i_ch)
{
    string Ahd_Online;
    string isOnline = "0x3";
    ...
    db_warn("getAhdInsertOnline online = %s\n",Ahd_Online.c_str());
    online_value = strtol(Ahd_Online.c_str() + 2, &nPtr, 16);
    db_warn("online_value:%d, (online_value >> (i_ch * 4)) & 0xF:0x%x\n", online_value, (online_value >> (i_ch * 4)) & 0xF);
    return (online_value >> (i_ch * 4)) & 0xF;
    #if 0
