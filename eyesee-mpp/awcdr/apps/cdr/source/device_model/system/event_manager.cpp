...
char const *const M_WAKEUP_AP =
    "/sys/devices/virtual/misc/sunxi-usb4g/usb4g-ctrl/m_wakeup_ap";
char const *const AP_4G_RST =
    "/sys/devices/virtual/misc/sunxi-usb4g/usb4g-ctrl/ap_4g_rst";
char const *const AP_4G_PWR =
    "/sys/devices/virtual/misc/sunxi-usb4g/usb4g-ctrl/ap_4g_pwr";
char const *const AP_WAKEUP_M =
    "/sys/devices/virtual/misc/sunxi-usb4g/usb4g-ctrl/ap_wakeup_m";
#define TEST_GSENSOR "/sys/devices/platform/gpio_sw.4/gpio_sw/PH13/data"
#define LIGHT_VALUE \
  "/sys/devices/virtual/misc/light-value/light-value/light_value"
#define FILE_ENABLE_ACC_SWITCH "/mnt/extsd/.tmp/.t/.dIsAbLeAcC"

char const *const IR_LED_FILE =
    "/sys/devices/platform/soc/gpioled/leds/ir_led/brightness";
...
EventManager::~EventManager() {
  if (event_loop_thread_id_ > 0) pthread_cancel(event_loop_thread_id_);

  if (m_ImapactEvent_thread_id > 0) pthread_cancel(m_ImapactEvent_thread_id);

  if (m_IrCheckFun_thread_id > 0) pthread_cancel(m_IrCheckFun_thread_id);
...
int EventManager::setIrLedBrightness(enum IRLedState IrLedState) {
  int ret = 0;
  if (access(IR_LED_FILE, F_OK) < 0) return -1;

  ret = WriteFileInt(IR_LED_FILE, IrLedState);
  if (ret < 0) db_error("write %s fail ,err = %d \n", IR_LED_FILE, ret);
  return ret;
}
...
void EventManager::RunAccEvent() {
  if (listen_fd_ >= 0)
    ThreadCreate(&event_loop_thread_id_, NULL, EventManager::EventLoopThread,
                 this);
  ThreadCreate(&m_ImapactEvent_thread_id, NULL, EventManager::CheckImpackEvent,
               this);
  ...
  ThreadCreate(&m_GpsEvent_thread_id, NULL, EventManager::startLocationReport,
               this);
#endif
#endif
}
...
void EventManager::RunBindFlagDetect() {
  ThreadCreate(&m_BindFlagEvent_thread_id, NULL,
               EventManager::CheckBindFlagEvent, this);
}

void EventManager::RunIrCheckFun() {
  ThreadCreate(&m_IrCheckFun_thread_id, NULL,
               EventManager::CheckIrCheckFunEvent, this);
}

int EventManager::configureSerialPort(int fd, int nSpeed, int nBits,
...
// first port is diag port ; second port is gps port; third port is AT commond
// port;
...
#else
  if ((access("/dev/ttyUSB0", F_OK) == 0) &&
      (access("/dev/ttyUSB1", F_OK) == 0) &&
      (access("/dev/ttyUSB2", F_OK) == 0))
#endif
    return 1;
  else
    return 0;
...
int EventManager::CheckSDCard() {
  ...
  if (WIFEXITED(status)) {
    if (WEXITSTATUS(status) == 0) {
      ...
      stat(SDCARD_NODE, &st);
      if ((st.st_mode & S_IRUSR) == S_IRUSR) return 1;
    }
  }
...
int EventManager::CheckSDCard_Cid() {
  ...
  if (NULL != cid_dir) closedir(cid_dir);
  if (NULL != mmc0_dir) closedir(mmc0_dir);
...
int EventManager::CheckNetLink(const char *interface) {
  ...
  if (ioctl(sockfd, SIOCGIFFLAGS, &ifr) < 0) {
    db_debug("ioctl SIOCGIFFLAGS error, interface[%s], %s", interface,
             strerror(errno));
    close(sockfd);
    return -1;
  }
...
int EventManager::NetLinkMsgHandler(struct sockaddr_nl *sockaddr,
...
int EventManager::ReadNetLinkEvent(int sockfd) {
  ...
  for (nlmsg = (struct nlmsghdr *)buf; NLMSG_OK(nlmsg, (unsigned int)status);
    ...
    /* Finish reading */
    if (nlmsg->nlmsg_type == NLMSG_DONE) return ret;
...
void *EventManager::EventLoopThread(void *context) {
  ...
  while (1) {
    ...
    switch (event) {
      ...
      case AHD_CAMERA_CONNECTED: {
        db_warn("AHD_CAMERA_CONNECTED:\n");
        em->Notify(MSG_AHD_CONNECT);
      } break;
      case AHD_CAMERA_DETACHED: {
        db_warn("AHD_CAMERA_DETACHED:\n");
        em->Notify(MSG_AHD_REMOVE);
      } break;
      default:
        break;
...
void edog_notify_cb_t(INT8U alarm, INT32U event, INT32U speed,
                      INT16U distance) {
...
static int send_packet(int sockfd) {
  ...
  ntp_head[0] =
      htonl(DEF_NTP_HEAD_LI | DEF_NTP_HEAD_VN | DEF_NTP_HEAD_MODE |
...
static int connect_to_server(const char *ntp_server) {
  ...
  memcpy(&(addr_dst.sin_addr.s_addr), host->h_addr_list[0],
         sizeof(addr_dst.sin_addr));
  // memcpy(&(addr_dst.sin_addr.s_addr), "61.216.153.104",
  // strlen("61.216.153.104"));
  addr_dst.sin_family = AF_INET;
  addr_dst.sin_port = htons(DEF_NTP_PORT);
#ifdef DEBUG_NTP
  db_msg("ntp server %s address:%s, port:%d\n", server,
         inet_ntoa(addr_dst.sin_addr), DEF_NTP_PORT);
#endif
...
int EventManager::USB4GModules_init() {
  ...
  while (ret <= 0) {
    ...
    if ((ptr = strstr(buf, "+ICCID: ")) != NULL) {
      if ((ptr1 = strstr(ptr, "\r\n")) != NULL) {
        strncpy(data, ptr + strlen("+ICCID: "),
                ptr1 - ptr - strlen("+ICCID: "));
        sim = data;
      }
    }
    if (!strcmp(sim.c_str(), "")) {
  ...
  while (ret <= 0) {
    ...
    USB4G_ComWrite(usb4g_fd, "AT^SYSCONFIG=19,7,1,2",
                   strlen("AT^SYSCONFIG=19,7,1,2"));
    usleep(5 * 1000);
    USB4G_ComWrite(usb4g_fd, "\r\n", 2);
    usleep(5 * 1000);
    ret = USB4G_ComRead(usb4g_fd, buf, sizeof(buf));
...
LOOP:
  ...
  while (ret <= 0) {
    ...
    USB4G_ComWrite(usb4g_fd, "AT^WAKESMSHEAD=1,12345678",
                   strlen("AT^WAKESMSHEAD=1,12345678"));
    usleep(5 * 1000);
    USB4G_ComWrite(usb4g_fd, "\r\n", 2);
    usleep(5 * 1000);
    ret = USB4G_ComRead(usb4g_fd, buf, sizeof(buf));
  ...
  while (ret <= 0) {
    ...
    USB4G_ComWrite(usb4g_fd, "AT^WAKECONFIG=1,0,30,iothub.xiaojukeji.com,1884",
                   strlen("AT^WAKECONFIG=1,0,30,iothub.xiaojukeji.com,1884"));
    usleep(5 * 1000);
    USB4G_ComWrite(usb4g_fd, "\r\n", 2);
    usleep(5 * 1000);
    ret = USB4G_ComRead(usb4g_fd, buf, sizeof(buf));
  ...
  while (ret <= 0) {
    ...
    USB4G_ComWrite(usb4g_fd, "AT^HEARTBEAT=12345678",
                   strlen("AT^HEARTBEAT=12345678"));
    usleep(5 * 1000);
    USB4G_ComWrite(usb4g_fd, "\r\n", 2);
    usleep(5 * 1000);
    ret = USB4G_ComRead(usb4g_fd, buf, sizeof(buf));
...
void *EventManager::USB4G_Init(void *context) {
  ...
  if (need_reset_apn) {
    while (1) {  // clean apn
      em->USB4G_ComWrite(em->usb4g_fd, "AT+CGDCONT=6,\"IPV4V6\",\"\"",
                         strlen("AT+CGDCONT=6,\"IPV4V6\",\"\""));
      usleep(5 * 1000);
      em->USB4G_ComWrite(em->usb4g_fd, "\r\n", 2);
      usleep(5 * 1000);
      ret = em->USB4G_ComRead(em->usb4g_fd, buf, sizeof(buf));
  ...
  while (1) {  // SWITCH PID
    em->USB4G_ComWrite(em->usb4g_fd, "AT+CUSBPIDSWITCH?",
                       strlen("AT+CUSBPIDSWITCH?"));
    usleep(5 * 1000);
    em->USB4G_ComWrite(em->usb4g_fd, "\r\n", 2);
    usleep(5 * 1000);
    ret = em->USB4G_ComRead(em->usb4g_fd, buf, sizeof(buf));
  ...
  if (em->mNeedSwitchPid) {
    ...
    while (ret <= 0) {  // SWITCH PID
      em->USB4G_ComWrite(em->usb4g_fd, "AT+CUSBPIDSWITCH=9011,1,1",
                         strlen("AT+CUSBPIDSWITCH=9011,1,1"));
      usleep(5 * 1000);
      em->USB4G_ComWrite(em->usb4g_fd, "\r\n", 2);
      usleep(5 * 1000);
      ret = em->USB4G_ComRead(em->usb4g_fd, buf, sizeof(buf));
  ...
  if (!strncmp(em->facturer.c_str(), " Thinkwill", strlen(" Thinkwill"))) {
    ...
    if (!strncmp(em->facturer.c_str(), " ZTE", strlen(" ZTE"))) {
      ...
      while (ret <= 0) {
        ...
        if ((ptr = strstr(buf, "+ZICCID:")) != NULL) {
          if ((ptr1 = strstr(ptr, "\r\n")) != NULL) {
            strncpy(data, ptr + strlen("+ZICCID:"),
                    ptr1 - ptr - strlen("+ZICCID:"));
            em->sim = data;
          }
        }
      }
  ...
  while (ret <= 0) {
    em->USB4G_ComWrite(em->usb4g_fd, "AT^SYSCONFIG=19,7,1,2",
                       strlen("AT^SYSCONFIG=19,7,1,2"));
    usleep(5 * 1000);
    em->USB4G_ComWrite(em->usb4g_fd, "\r\n", 2);
    usleep(5 * 1000);
...
LOOP:
  ...
  while (ret <= 0) {
    em->USB4G_ComWrite(em->usb4g_fd, "AT^WAKESMSHEAD=1,12345678",
                       strlen("AT^WAKESMSHEAD=1,12345678"));
    usleep(5 * 1000);
    em->USB4G_ComWrite(em->usb4g_fd, "\r\n", 2);
    usleep(5 * 1000);
  ...
  while (ret <= 0) {
    em->USB4G_ComWrite(
        em->usb4g_fd, "AT^WAKECONFIG=1,0,30,iothub.xiaojukeji.com,1884",
        strlen("AT^WAKECONFIG=1,0,30,iothub.xiaojukeji.com,1884"));
    usleep(5 * 1000);
    em->USB4G_ComWrite(em->usb4g_fd, "\r\n", 2);
    usleep(5 * 1000);
  ...
  while (ret <= 0) {
    ...
    if (strcmp(username.c_str(), "") != 0 && strcmp(secret.c_str(), "") != 0) {
      ...
      snprintf(atcmd, sizeof(atcmd), "AT+MQTTCONFIG=%s,%s",
               decode_username.c_str(), decode_secret.c_str());
      db_msg("atcmd:%s\n", atcmd);
      em->USB4G_ComWrite(em->usb4g_fd, atcmd, strlen(atcmd));
    } else {
      db_msg("atcmd not send\n");
      break;  // not send AT+MQTTCONFIG,else 4G will connect with this invalid
              // user
      // send Invaild user secret
      em->USB4G_ComWrite(em->usb4g_fd, "AT+MQTTCONFIG=12345678,12345678",
                         strlen("AT+MQTTCONFIG=12345678,12345678"));
    }
    usleep(5 * 1000);
    em->USB4G_ComWrite(em->usb4g_fd, "\r\n", 2);
    usleep(5 * 1000);
  ...
  bzero(buf, sizeof(buf));
  while (ret <= 0) {
    em->USB4G_ComWrite(em->usb4g_fd, "AT^HEARTBEAT=12345678",
                       strlen("AT^HEARTBEAT=12345678"));
    usleep(5 * 1000);
    em->USB4G_ComWrite(em->usb4g_fd, "\r\n", 2);
    usleep(5 * 1000);
...
// printf("GsensorManager::GetInstance()->getImpactStatus() :%d
// OverFlag:%d\n", status, OverFlag);
#if 0
    if (status && OverFlag) {
      ...
      db_error(
          "CheckImpackEvent MSG_IMPACT_HAPPEN++++++++++++++ curWinId=%d "
          "sd_status=%d \n",
          curWinId, sd_status);
      OverFlag = false;
      ...
      if (curWinId == WINDOWID_PREVIEW) {
        if (sd_status > 0) {
          ...
          db_error(
              "CheckImpackEvent MSG_IMPACT_HAPPEN++++++++++++++ NO SD Card!!!");
        }
      } else {
        db_error(
            "CheckImpackEvent is not WINDOWID_PREVIEW,  not deal impact video "
            "\n");
      }
  ...
  while (1) {
    ...
    if (!em->CheckttyUsb()) {
      ...
      if ((ptr = strstr(buf, "+CSQ: ")) != NULL) {
        ptr1 = getStrBehindComma(ptr, 1);
        if (ptr1 != NULL) {
          strncpy(data, ptr + strlen("+CSQ: "),
                  ptr1 - ptr - strlen("+CSQ: ") - strlen(","));
...
void EventManager::SetUSB4GModuleStandby(bool flag) {
  ...
  if (flag) {
    ...
    // system("echo 1 >
    // /sys/devices/virtual/misc/sunxi-usb4g/usb4g-ctrl/enable_acc");
...
int EventManager::IRledCtrl(bool flag) {
  ...
  if ((!loop_rec->RecorderIsBusy()) &&
      (!short_rec->RecorderIsBusy()))  //后台录像关闭
  {
    if (IrisOpen) {
      db_error(
          "==============black recorder is stop,close ir_led==============");
      setIrLedBrightness(IRLedOff);
      IrisOpen = false;
      IrisOpen_flag = false;
    }
  ...
  if (IrisOpen_flag == true) return 0;
  if (flag) {
...
int EventManager::GetAccResumeStatus() {
  ...
  db_msg("by hero GetAccResumeStatus = %d",
         menuconfiglua->GetMenuIndexConfig(SETTING_ACC_RESUME));
...
void EventManager::ResetAccStatus() {
  ...
  if (is_enable_acc == 1) {
    MenuConfigLua *menuconfiglua = MenuConfigLua::GetInstance();
    menu_config_value =
        menuconfiglua->GetMenuIndexConfig(SETTING_ACC_SWITCH);  //更新配置选项
    dc_state = GetDCconnetStatus();                  //从dc status获取状态
    CheckEvent(EventManager::EVENT_ACC, acc_state);  //从acc status获取状态
    if (((wakeup_event_id & CPUS_WAKEUP_AC) ||
         (wakeup_event_id & CPUS_WAKEUP_USB)) &&
        (dc_state == 0)) {  // dc模式下dc电源被拔除时系统唤醒
      ...
      return;
#endif
    } else if (((wakeup_event_id & CPUS_WAKEUP_AC) ||
                (wakeup_event_id &
                 CPUS_WAKEUP_USB))  // acc模式下接入dc时系统唤醒
               && (menu_config_value == 0) && (acc_state == 0)) {
      db_error(
          "===========wake up id is ac,menu config is acc,but acc is no "
          "in===========");
#if 0
			Notify(MSG_ACCOFF_HAPPEN);
			db_error("send msg MSG_ACCOFF_HAPPEN");
  ...
  if ((wakeup_event_id & CPUS_WAKEUP_AC) ||
      (wakeup_event_id & CPUS_WAKEUP_USB)) {  // DC OFF
...
void *EventManager::CheckAccEvent(void *context) {
  ...
  int gps_state = 0;
  //    int is_enable_acc = 1;  //check FILE_ENABLE_ACC_SWITCH to change this
  //    status
  MenuConfigLua *menuconfiglua = MenuConfigLua::GetInstance();
#ifdef MEM_DEBUG
  int count = 0;
      ...
      if (menu_config_value == 1) {
        if (dc_state == 1) {
          ...
          if (em->RemoteActionDone == 2) {
            ...
            //		     			db_msg("Remote Action Done  set value
            //time
            // out,notify MSG_ACCOFF_HAPPEN!!!!");
            if (em->mNotify_acc_off_flag == false) {
              db_msg("CheckAccEvent DC out");
              em->Notify(MSG_ACCOFF_HAPPEN);
              db_msg("notify MSG_ACCOFF_HAPPEN");
...
static double cal_clongitude_latitude_value(
    const char *data) {  // ddmm.mmmm or dddmm.mmmm ->dd.dddddd ddd.dddddd
  int value_high;
        ...
        if (ptr[0] != '0') {
          ...
          if (ptr != NULL) {
            if (ptr[0] != ',') {
              ...
              strncpy(temp, ptr, 6);
              sscanf(temp, "%2d%2d%2d.%3d", &mLocationInfo.utc.hour,
                     &mLocationInfo.utc.min, &mLocationInfo.utc.sec,
                     &mLocationInfo.utc.msec);
            }
          }
          ...
          if (ptr != NULL) {
            if (ptr[0] != ',') {
              ...
              tmp_value = cal_clongitude_latitude_value(
                  temp);  // ddmm.mmmm -> dd.dddddd
            }
          }
...
#ifdef DEBUG_GPS
          db_error("==========em->mLocationInfo.GpsSignal %d==========",
                   mLocationInfo.GpsSignal);
#endif
          if (mLocationInfo.GpsSignal <= 4) {
            mGpsSignal = 1;
          } else if ((mLocationInfo.GpsSignal > 4) &&
                     (mLocationInfo.GpsSignal <= 6)) {
            mGpsSignal = 2;
...
#ifdef DEBUG_GPS
          db_msg("GPS utc is %d:%d:%d,%d-%d-%d\n", mLocationInfo.utc.hour,
                 mLocationInfo.utc.min, mLocationInfo.utc.sec,
                 mLocationInfo.utc.year, mLocationInfo.utc.mon,
                 mLocationInfo.utc.day);
          db_msg("GPS location is %c %f,%c %f, altitude %f\n", mLocationInfo.NS,
                 mLocationInfo.latitude, mLocationInfo.EW,
                 mLocationInfo.longitude, mLocationInfo.altitude);
#endif
          if (mGpsSignal) {
            if (rmcflag && synctimeflag) {
              int timezone = MenuConfigLua::GetInstance()->GetTimezone();
              UTCsetSystemTime(
                  mLocationInfo.gtm.tm_year, mLocationInfo.gtm.tm_mon,
                  mLocationInfo.gtm.tm_mday, mLocationInfo.utc.hour,
                  mLocationInfo.utc.min, mLocationInfo.utc.sec, timezone);
...
void EventManager::UTCsetSystemTime(int year, int month, int day, int hour,
                                    int minute, int second, int tmzone) {
  struct timeval tv;
  struct timezone tz;
  struct tm tm_time;
  struct tm *p;
  ...
  db_error("GPS UTC:    %d/%d/%d %d:%d:%d %d", year, month, day, hour, minute,
           second, tmzone);  // 2019/6/13 10:56:7 0
  ...
  tm_time.tm_yday = 0;
  tm_time.tm_isdst = 0;
  db_error("system  UTC: %d/%d/%d %d:%d:%d", p->tm_year, p->tm_mon, p->tm_mday,
           p->tm_hour, p->tm_min,
           p->tm_sec);  // 70/0/1 0:0:0 表示1970/1/01 00:00:00
  db_error("tm_time UTC: %d/%d/%d %d:%d:%d", tm_time.tm_year, tm_time.tm_mon,
           tm_time.tm_mday, tm_time.tm_hour, tm_time.tm_min,
           tm_time.tm_sec);  // 70/0/1 0:0:0 表示1970/1/01 00:00:00
#if 0
...
#endif
  {
    ...
    tv.tv_sec = mktime(&tm_time) +
                tmzone * 3600;  // 因为mktime()已经根据当前时区进行计算,
                                // 如果是GMT+8则减去了288000,
                                // 加回来得到GMT+0的时间
    tv.tv_usec = 0;
    tz.tz_minuteswest = 0;  // 和Greenwich 时间差了多少分钟
  ...
  // struct timezone tz;
  // gettimeofday(&tv, &tz);
  // db_error("tv_sec = %d, tv_usec = %d, tz_minuteswest = %d, tz_dsttime =
  // %d\n",
  ...
  db_error("new system UTC: %d/%d/%d %d:%d:%d", p->tm_year, p->tm_mon,
...
// strncpy(mRMCData,"$GPRMC,100539.000,A,2232.8557,N,11355.7438,E,0.13,309.62,031209,,*10",
// sizeof(mRMCData));
...
#ifdef DEBUG_GPS
              db_msg("hour:%d min:%d sec:%d\n", mLocationInfo.gtm.tm_hour,
                     mLocationInfo.gtm.tm_min, mLocationInfo.gtm.tm_sec);
#endif
            }
          }
...
#ifdef DEBUG_GPS
              db_msg("year:%d mon:%d mday:%d\n", mLocationInfo.gtm.tm_year,
                     mLocationInfo.gtm.tm_mon, mLocationInfo.gtm.tm_mday);
#endif
              rmcflag = 1;
            }
          }
  ...
  while (1) {
    ...
    if ((ptr = strstr(temp_ptr, "$GPGSV")) != NULL) {
      if ((ptr2 = strstr(ptr, "\r\n")) != NULL) {
        ...
        for (loop_iii = 0;
             loop_iii < (current_gpgsv < total_gpgsv
                             ? 4
                             : (total_sa % 4) == 0 ? 4 : (total_sa % 4));
             loop_iii++) {
          // db_msg("GPRMC data valiable status:%c",ptr[0]);
          if ((ptr = getStrBehindComma(mGSVData, 4 + loop_iii * 4)) != NULL) {
            if (ptr != NULL) {
              if (ptr[0] != ',') {
                ...
                if ((ptr = getStrBehindComma(mGSVData, 7 + loop_iii * 4)) !=
                    NULL) {
                  if (ptr != NULL) {
                    if (ptr[0] != ',') {
                      bzero(tmpsa_nr, sizeof(tmpsa_nr));
        ...
        if (current_gpgsv >= total_gpgsv) break;
      } else
        break;
...
void EventManager::dosavegpslog(char *temp) {
  ...
  sprintf(gpsbuff, "%04d-%02d-%02d  %02d:%02d:%02d   %s", p->tm_year + 1900,
          p->tm_mon + 1, p->tm_mday, p->tm_hour, p->tm_min, p->tm_sec, temp);
...
+ wr < rd 说明 写指针在后(已经回环)
+ */
int EventManager::GetGPSrecord(char *buff, char *pwr, char *&prd,
                               char *bufret) {
#if 1
  *bufret = 0;
  if (pwr == prd) {
  ...
  while (1) {
    ch = *prx;
    prx++;
    if (prx >= buff + GPSBUF_SIZE)
      prx = buff;  // 读指针超出环形缓冲区末尾,返回头部
    switch (ch) {
      case '$':
        bitflag = 1;
        *tmpx++ = ch;
...
char *gps_sim_data[16] = {
    "$GPGGA,075301.305,2237.3387,N,11404.3147,E,1,7,1.04,126.7,M,-2.1,M,,*"
    "4E\r\n",
...
//		}
  ...
  loop:
    ...
    if (em->gpslogfd < 0) {
      if ((em->gpslogfd =
               open("/mnt/extsd/gpslog.txt", O_RDWR | O_CREAT, 0666)) < 0) {
...
//							em->mGpsSignal = em->mLocationInfo.GpsSignal / 2;
      ...
      // printf("buff:|%s|\n",buf);
      while (pr != pw) {
        ret = em->GetGPSrecord(buf, pw, pr,
                               temp);  // 如果有有效的数据会改变pr的值
...
int EventManager::setGPSData() {
  ...
  // strncpy(p_gpsInfo.latitude,mLocationInfo.latitude.c_str(),
  // sizeof(p_gpsInfo.latitude));
  // strncpy(p_gpsInfo.longitude,mLocationInfo.longitude.c_str(),
  // sizeof(p_gpsInfo.longitude));
  //    strncpy(p_gpsInfo.car_speed, mLocationInfo.speed.c_str(),
  //    sizeof(p_gpsInfo.car_speed));
  //    strncpy(p_gpsInfo.latitude, mLocationInfo.latitude.c_str(),
  //    sizeof(p_gpsInfo.latitude));
  //   strncpy(p_gpsInfo.longitude, mLocationInfo.longitude.c_str(),
  //   sizeof(p_gpsInfo.longitude));
  // p_gpsInfo.net_state = GetNetStatus();
  // strncpy(p_gpsInfo.timestamp, std::to_string(time(0)).c_str(),
  //         sizeof(p_gpsInfo.timestamp));
