...
int8_t create_dir(const char *dirpath) {
  if (filesystem::create_directories(dirpath)) {
    db_error("mkdir '%s' filed: %s", dirpath, strerror(errno));
    return -1;
  }
  filesystem::permissions(dirpath, filesystem::perms::all);
  return 0;
}
...
uint32_t calc_record_time_by_size(uint32_t bitrate, uint32_t size) {
  ...
  if (onehour_size < MAX_ONE_FILE_SIZE) {
    ...
    rec_time = MAX_ONE_FILE_SIZE / rate;
  }
...
// 预分配10%余量, 防止实际写入大小超过计算的预分配大小
  ...
  if (redundancy_size < (10 * (1 << 20))) redundancy_size = 10 * (1 << 20);
...
bool verify_bin_md5() {
  ...
  if (access("/mnt/extsd/.sdvcam", F_OK) != 0) {
    db_info(
        "run application from rootfs, no need verify md5, just check build "
        "version");
    return true;
  }
...
int ispsave_file_filter(const struct dirent *dir) {
  ...
  stat(dir->d_name, &buf);
  if (!S_ISDIR(buf.st_mode)) {  // isp0_3840_2160_25_ctx_saved.bin
