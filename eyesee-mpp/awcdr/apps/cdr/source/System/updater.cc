...
const char* System::Updater::__dir_path__ = "/mnt/extsd/";
const char* System::Updater::__file_name__ = "ota.tar";
const char* System::Updater::__temp_dir_name__ = "/tmp/updating/";

System::Updater::Updater() : logger(std::string{"Updater"}) {
...
auto System::Updater::CheckFirmwareFileExist() -> bool {
  return filesystem::exists(_firmware_file_path_);
}

void System::Updater::StartUpdate() {
  ...
  try {
    ...
    filesystem::remove_all(_firmware_file_path_);
    for (const auto& file_name : file_list) {
    ...
    Log(LogLevel::Error) << GetMessageIn(error) << '\n';
  }
  System::CommandLine::Send("reboot");
}

auto System::Updater::GetFirmwareFileList(std::string tar_path)
    -> std::vector<std::string> {
  std::vector<std::string> file_list;
  System::CommandLine::Send("tar -tf " + tar_path, [&](auto file_name) {
    file_list.push_back(file_name);
  });
  return file_list;
}

auto System::Updater::GetMessageIn(const Error& error) -> std::string {
  std::string message;
...
void System::Updater::UnpackFileTo(std::string name, std::string from,
                                   std::string to_dir) {
  try {
    System::CommandLine::Send("tar -xv -f " + from + " " + name + " -C " +
                              to_dir);
  } catch (bool isFailed) {
    throw System::Updater::Error{
      file_name : name,
...
auto System::Updater::GetFileMD5In(filesystem::path file_path) -> std::string {
  std::string md5_string;
  try {
    System::CommandLine::Send(
...
void System::Updater::CheckImageMD5(std::string file_path) {
  try {
...
auto System::Updater::GetBurnCommand(Burntable burn_part,
  ...
  switch (burn_part) {
    ...
    case Burntable::Boot:
      return "dd if=" + image_path + " bs=1M | pv -s " +
             std::to_string(filesystem::file_size(image_path)) +
             " | dd of=/dev/by-name/boot conv=fsync";
    case Burntable::RootFS:
      return "dd if=" + image_path + " bs=1M | pv -s " +
             std::to_string(filesystem::file_size(image_path)) +
             " | dd  of=/dev/by-name/rootfs conv=fsync";
  }
}

void System::Updater::Burn(Burntable burn_part) {
  ...
  try {
    System::CommandLine::Send(
        ...
        [this](auto message) { this->Log(LogLevel::Info) << message << '\n'; });
  } catch (bool isFailed) {
    throw System::Updater::Error{
      file_name : file_name,
  ...
  if (auto delegate = delegate_.lock())
    delegate->BurningProgress(burn_part, 100);
}
