...
#else
 #define WPA_SUP_CFG_PATH        "/var/wpa_supplicant/wlan0"
 #define WPA_SUP_CFG_FILE        "/etc/wpa_supplicant.conf"
#endif
...
//max+
#define WPA_SUPPLICANT_BASE_CONF	"ctrl_interface=/var/wpa_supplicant\n"\
									"update_config=1\n"\
									"\n"\
									"network={\n"\
									"    ssid=\"%s\"\n"\
...
int wifi_sta_init(void)
{
    ...
    }else if((ret = access("/etc/firmware/ap6212", F_OK)) == 0){
        strcpy(g_dev_name, "ap6212");
    }
    ...
    system(buf);
#else
	snprintf(buf, sizeof(buf)-1, "insmod /lib/modules/4.9.118/8189fs.ko");
    system(buf);
	usleep(200*1000);
...
int wifi_sta_exit(void)
{
    ...
    system(buf);
#else
	snprintf(buf, sizeof(buf)-1, "killall -9 wpa_supplicant");
    system(buf);
	usleep(200*1000);
	snprintf(buf, sizeof(buf)-1, "ifconfig wlan0 down");
    system(buf);
	usleep(200*1000);
	snprintf(buf, sizeof(buf)-1, "rmmod 8189fs");
    system(buf);
	usleep(200*1000);
#endif
...
int wifi_sta_conf(const char *ssid, const char *pwd)	//max+
{
	int   ret       = 0;
	char  buf[1024] = {0};
    FILE  *fp       = NULL;
	...
	pthread_mutex_lock(&g_operate_lock);
	fp = fopen(WPA_SUP_CFG_FILE, "wb+");
    if (NULL == fp) {
        STA_ERR_PRT ("Create %s fail! errno:%d  error:%s\n",
                                    WPA_SUP_CFG_FILE, errno, strerror(errno));
		pthread_mutex_unlock(&g_operate_lock);
        return -1;
    }
    memset(buf, 0, sizeof(buf));
    ret = snprintf(buf, sizeof(buf)-1, WPA_SUPPLICANT_BASE_CONF, ssid, pwd);
    if (ret <= 0) {
        STA_ERR_PRT ("Do snprintf WPA_SUPPLICANT_BASE_CONF fail! ret:%d\n", ret);
		pthread_mutex_unlock(&g_operate_lock);
        return -1;
    }	
    ret = fwrite(buf, ret, 1, fp);
    if (ret <= 0) {
        STA_ERR_PRT ("fwrite fail! errno:%d  error:%s\n", errno, strerror(errno));
		pthread_mutex_unlock(&g_operate_lock);
        return -1;
    }
